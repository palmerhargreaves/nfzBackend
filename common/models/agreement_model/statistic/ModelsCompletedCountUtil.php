...
class ModelsCompletedCountUtil {
    ...
    const SENT_TO_DESIGNER = 'sent_to_specialist';
    ...
    private function getModelsCheckedList($date, $only_by_day = false, $closure = null) {
        ...
        $model_specialist_actions = ['declined_by_specialist', 'accepted_by_specialist', self::SENT_TO_DESIGNER];
        $model_actions = array_merge(['declined', 'accepted'], $model_specialist_actions);

        $check_models_list = [ ];

        //Проходим по полученному списку логов, для определения кем проверена заявка - менеджером или менеджером / дизайнером
        $results = [];

        $pre_results = $this->makeQuery($date, [], $object_type_by_model, $model_actions);
        foreach ($pre_results as $item) {
            $work_type = self::MANAGER;
            if (in_array($item['action'], $model_specialist_actions)) {
                $work_type = self::MANAGER_DESIGNER;
            }

            $results[] = array('work_type' => $work_type, 'action' => $item['action'], 'created_at' => $item['created_at'], 'object_id' => $item['object_id']);
        }
        ...
        if (!empty($result_manager_designer_models_ids)) {
            ...
            foreach ($result_manager_designer as $result_item) {
                //Если заявка отправлена специалисту на проверку, пропускаем это действие
                if ($result_item['action'] == self::SENT_TO_DESIGNER) {
                    continue;
                }
     ...
     * @internal param string $check_type
     */
    private function makeQuery($date = '', $object_ids = [], $object_type = '', $actions = []) {
        $query = Log::find()
            ->select(['object_id', 'log.created_at', 'action'])
